...
#variaveis de tempo
Tsimula = 420   # t em segundos de simulação
...
def Vsaida(Vc,thetaVC,V,I,FP,signal):
   ...
   #Impedância virtual - Valores para testes
    #Rs = 1;
    #Ls = 0.003;

   # Rs = 0.00642;  #valores da simulação plecs
    #Ls = 0.00083; 
...
def sinalFP(vetV,vetC):
    ...
    #verifica o primeiro sinal do elemento do vetor
    if(corr[0]>0):
     sinal_c = 1
    else:
     sinal_c = -1
   ...
   #Verifica se FP esta atrasado ou adiantado
    if (sinal_c>0 and sinal_t<0):
        sinal = -1   #FP capacitivo corrente atrasada
    else:
        if (sinal_c<0 and sinal_t>0):
            sinal = 1   #FP indutivo corrente adiantada 
...
#Tensão da rede
vc = 220;   #modulo 100 v
#vc = 225;   # -> SOBRETENSÃO DE 5 V +- 2,30% de elevação na tensão
#vc = 222;   # -> SOBRETENSÃO DE 2 V +- 0,909% de elevação na tensão
   ...
   # while cont2==0:
     ...
     ##################################################################################################################
       #verifica o sinal do fator de potência
        ...
        ################Filtro média móvel ###################
        #MM = 8 #coeficiente do filtro média móvel  (Valor original utilizado é 8)

        MM = 8 #valor de teste para ver se fica mais rápido a execução do código
       ...
       # print('tensao senoidal = ',vetorVsen)
       # print('tensao corrente = ',vetorCsen)
    ...
    ######### Fecha o arquivo CSV ###################
    #w.close() /Isso Não esta sendo utilizado !
    ###f.close()
